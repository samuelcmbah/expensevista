"use client" //tells nextjs that this is a client component
...
const SelectTrigger = React.forwardRef<React.ComponentRef<typeof SelectPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger>>
  (({ className, children, ...props }, ref) => (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex w-full items-center justify-between rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700",  
        "transition focus:outline-none focus:ring-2 focus:ring-green-100 focus:border-green-300 disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      {...props}
    >
...
//when dropdown has more items than can fit, these buttons allow scrolling
  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        ...
        // make dropdown same width and aligned with trigger
...
//optional label inside the dropdown(like "Select an option")
...
const SelectItem = React.forwardRef<React.ComponentRef<typeof SelectPrimitive.Item>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item>>
  ...
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-pointer select-none items-center rounded-md px-2 py-1.5 text-sm text-gray-700 outline-none focus:bg-green-50 focus:text-green-700 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
